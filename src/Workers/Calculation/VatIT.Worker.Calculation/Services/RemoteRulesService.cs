...
namespace VatIT.Worker.Calculation.Services;
...
public class RemoteRulesService
{
    ...
    public async Task RefreshAsync()
    {
        try
        {
            ...
            var res = await client.GetAsync("/admin/rules/calculation");
            if (!res.IsSuccessStatusCode)
            {
                _logger.LogDebug("No remote rules for calculation: {Status}", res.StatusCode);
                return;
            }
            var json = await res.Content.ReadAsStringAsync();
            var doc = JsonDocument.Parse(json);
            Latest = doc.RootElement.Clone();
            _logger.LogDebug("Fetched remote calculation rules");
        }
        catch (Exception ex)
        {
            _logger.LogDebug(ex, "Failed to refresh calculation rules");
        }
    }
}
