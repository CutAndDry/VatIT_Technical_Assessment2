...
namespace VatIT.Worker.Validation.Services;
...
public class RemoteRulesService
{
    ...
    public async Task RefreshAsync()
    {
        try
        {
            ...
            var res = await client.GetAsync("/admin/rules/validation");
            if (!res.IsSuccessStatusCode)
            {
                _logger.LogDebug("No remote rules for validation: {Status}", res.StatusCode);
                return;
            }
            var json = await res.Content.ReadAsStringAsync();
            var doc = JsonDocument.Parse(json);
            Latest = doc.RootElement.Clone();
            _logger.LogDebug("Fetched remote validation rules");
        }
        catch (Exception ex)
        {
            _logger.LogDebug(ex, "Failed to refresh validation rules");
        }
    }
}
